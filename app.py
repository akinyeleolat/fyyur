...
def show_venue(venue_id):
    ...
    try:
        ...
        result_past_shows_count = Show.query.join(Venue, Venue.id == Show.venue_id).filter(
            Show.start_time < date.today()).count()
        result_upcoming_shows_count = Show.query.join(Venue, Venue.id == Show.venue_id).filter(
            Show.start_time > date.today()).count()

        result_past_shows = Show.query.join(Venue, Venue.id == Show.venue_id) \
            .join(Artist, Artist.id == Show.artist_id) \
            .add_columns(Artist.id.label('artist_id'), Artist.name.label('artist_name'),
...
def show_artist(artist_id):
    ...
    try:
        ...
        result_past_shows_count = Show.query.join(Artist, Artist.id == Show.artist_id).filter(
            Show.start_time < date.today()).count()
        result_upcoming_shows_count = Show.query.join(Artist, Artist.id == Show.artist_id).filter(
            Show.start_time > date.today()).count()

        result_past_shows = Show.query.join(Artist, Artist.id == Show.venue_id) \
            .join(Venue, Venue.id == Show.artist_id) \
            .add_columns(Venue.id.label('venue_id'), Venue.name.label('venue_name'),
